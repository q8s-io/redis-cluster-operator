...
func buildClusterStatus(clusterInfos *redisutil.ClusterInfos, pods []*corev1.Pod,
	cluster *redisv1alpha1.DistributedRedisCluster, reqLogger logr.Logger, pvcCtrl k8sutil.IPvcControl, pvCtrl k8sutil.IPVController) *redisv1alpha1.DistributedRedisClusterStatus {
	...
	for _, pod := range pods {
		...
		if len(pod.OwnerReferences) > 0 {
			if pod.OwnerReferences[0].Kind == "StatefulPod" {
				newNode.StatefulPod = pod.OwnerReferences[0].Name
			}
		}
