...
func newRedisCluster(infos *redisutil.ClusterInfos, cluster *redisv1alpha1.DistributedRedisCluster) (*redisutil.Cluster, redisutil.Nodes, error) {
	...
	for _, node := range cluster.Status.Nodes {
		if rNode, ok := rCluster.Nodes[node.ID]; ok {
			...
			rNode.StatefulPod = node.StatefulPod
		}
	}
...
func (w *waitPodTerminating) Handler() error {
	...
	labels[redisv1alpha1.StatefulPodLabel] = w.statefulPod
	podList, err := w.statefulPodController.GetStatefulPodPodsByLabels(w.cluster.Namespace, labels)
	if err != nil {
		return err
	}
...
func (w *waitStatefulSetUpdating) Handler() error {
	labels := getLabels(w.cluster)
	stsList, err := w.statefulPodController.ListStatefulPodByLabels(w.cluster.Namespace, labels)
	if err != nil {
		return err
	}
	for _, sts := range stsList.Items {
		if int32(len(sts.Status.PodStatusMes)) != (w.cluster.Spec.ClusterReplicas + 1) {
			return nil
		}
	}
	return fmt.Errorf("statefulPod still not updated")
}
