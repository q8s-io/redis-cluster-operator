...
func newReconciler(mgr manager.Manager, execer exec.IExec) reconcile.Reconciler {
	...
	reconiler.statefulPodController = k8sutil.NewStatefulPodController(reconiler.client)
	...
	reconiler.podController = k8sutil.NewPodController(reconiler.client)
	reconiler.pvController = k8sutil.NewPVController(reconiler.client)
...
type ReconcileDistributedRedisCluster struct {
	...
	statefulPodController k8sutil.IStatefulPodControl
	...
	pvController          k8sutil.IPVController
}
...
func (r *ReconcileDistributedRedisCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.markSlavePV(instance); err != nil {
		return reconcile.Result{}, err
	}
	...
	redisClusterPods, err := r.statefulPodController.GetStatefulPodPodsByLabels(instance.Namespace, matchLabels)
	if err != nil {
	...
	password, err := statefulpods.GetClusterPassword(r.client, instance)
	if err != nil {
	...
	// master 加载数据完毕,创建 slave
	if len(instance.Spec.PVNames) != 0 && instance.Spec.ClusterReplicas == 0 {
		r.getSlaveNum(instance)
	...
	if instance.IsRestoreFromBackup() && instance.IsRestoreRunning() {
		...
		if err := r.ensurer.UpdateRedisStatefulPods(instance, getLabels(instance)); err != nil {
			return reconcile.Result{}, err
		}
	...
	status := buildClusterStatus(clusterInfos, ctx.pods, instance, reqLogger, r.pvcController, r.pvController)
	if status == nil {
		return reconcile.Result{}, nil
	}
	...
	newStatus := buildClusterStatus(newClusterInfos, ctx.pods, instance, reqLogger, r.pvcController, r.pvController)
	if newStatus == nil {
		return reconcile.Result{}, nil
	}
...
func (r *ReconcileDistributedRedisCluster) isScalingDown(cluster *redisv1alpha1.DistributedRedisCluster, reqLogger logr.Logger) bool {
	stsList, err := r.statefulPodController.ListStatefulPodByLabels(cluster.Namespace, getLabels(cluster))
	if err != nil {
